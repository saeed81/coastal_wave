MODULE solver 
  ...
  REAL(8), PUBLIC, PARAMETER :: ratio = 3.0
...
CONTAINS
  ...
  SUBROUTINE rhs( pdt, pdx, pdy, pfrhs, pgrhs )
    ...
    zca = 1.0 +( zphi ) / (1.0 - omega * omega) * bp**(-3.0*alpha)*pdt**(2.0*alpha-2.0)*alpha*(2.0-4.0*alpha+(wn/omega)) 

    zcb = (1.0 + alpha) / pdt - zphi * bp**(-2.0*alpha)*pdt**(2.0*alpha-1.0) *alpha* (bp**(-alpha)  &
         & * pdt**(alpha-2.0) / (1.0 -omega * omega) *(4.0*alpha*(alpha-1.0) + 2.0 + (3.0-7.0*alpha)*(wn/omega))+1.0) 

    zcd =  ( wn * wn + wn * alpha / omega ) / (pdt * pdt) +  (1.0-omega*omega ) & 
         & / (zcc * pdt**(alpha) ) + zphi *&
         &  bp**(-3.0*alpha)  * alpha / (1.0 - omega * omega) * pdt**(alpha) *( pdt**(2.0*alpha-4.0) * wn / omega * &
    ( 4.0 * alpha * (2.0-alpha ) + &
         &3.0*alpha* wn / omega - 2.0 * wn * omega + wn * wn - 4.0 )- pdt**(alpha -2.0) * (1.0 - omega * omega) / (bp**(-alpha)))
